...
TASK="${TASK:-stack the red cube on top of the blue cube}"
...
if [[ -z "${ROBOT_PORT}" || -z "${CAMERA1_INDEX}" ]]; then
    echo "Set ROBOT_PORT and CAMERA1_INDEX. Example:" >&2
    echo "  ROBOT_PORT=/dev/ttyACM0 CAMERA1_INDEX=0 bash scripts/run_lerobot_pi05_so101_client.sh" >&2
    exit 1
fi
...
if [[ -n "${CAMERA2_INDEX:-}" ]]; then
    ARGS+=(--camera2_index "${CAMERA2_INDEX}" --camera2_key "${CAMERA2_KEY:-camera2}")
fi
